...
class GameController extends Controller
{
    ...
    public function sendItems($bets, $user, $chance){						
		...
		if (strpos(strtolower(' '.$name),  strtolower(config('mod_game.sitename'))) != false) $commission = $commission - config('mod_game.comission_site_nick');
    ...
    public function newBet(){
        ...
        foreach ($data as $newBetJson) {
			...
			if ($user->ban == 0){
				...
				$vip = 0;
				if (strpos(strtolower(' '.$user->username),  strtolower(config('mod_game.sitename'))) != false) $vip = 1;
	...
	public function curcomm(){
		...
		if (!is_null($this->user)){
			...
			if (strpos(strtolower(' '.$this->user->username),  strtolower(config('mod_game.sitename'))) != false) $my_comission = $my_comission - config('mod_game.comission_site_nick');
		}
