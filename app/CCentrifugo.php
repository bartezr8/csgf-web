...
class CCentrifugo{
    ...
    public static function publish($channle, $messageData){
        $centrifugo = new Centrifugo(env('CENT_SCHEME_API').'://'.env('CENT_HOST').env('CENT_URL_API'), env('CENT_SECRET'), ['redis' => ['host'=>env("CENT_REDIS_HOST"),'port'=>env("CENT_REDIS_PORT"),'db'=>env("CENT_REDIS_DB"),'timeout'=>env("CENT_REDIS_TIMEOUT")],'http'=>[CURLOPT_TIMEOUT=>5]]);
        try {
